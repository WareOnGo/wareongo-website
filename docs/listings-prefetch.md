...
A separate existing issue was found while testing direct filtered URLs. Loading `/listings?type=PEB` against the static site produces React hydration errors 418/423 because static HTML contains the unfiltered grid while the first client render uses URL filters. The same two failing cases were reproduced in an isolated build with the original `Listings.tsx` from commit `1ea0412`, with preloading absent. This change leaves that behavior untouched. Empty and single-page prefetch checks apply filters through the visible controls after hydration; they still fail on any unexpected browser JavaScript error. The hydration finding needs a separate fix.
