...
Updated 2026-09-22: next-page data preparation also waits until the currently visible photos have completed. Browser idle time alone does not establish that the network is idle. Image load/error events and scrolling/resizing reconsider readiness; the existing connection restrictions, query identity, cancellation and Next-request adoption remain in place. The same check gates photo preloading. See [listing performance validation](./listing-performance-review-2026-09-22.md).
